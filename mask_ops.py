"""
Mask and compositing ops for ComfyUI-311-Inpaint.

Workflow type ids are unchanged so existing graphs keep loading.
Display names use the 311 suffix. Category is 311/Inpaint.

Copyright (c) 2023 Jacob Segal
MIT License — see THIRD_PARTY_NOTICE.txt
"""
...
def tensor2rgba(t: torch.Tensor) -> torch.Tensor:
    ...
    if (len(size) < 4):
        ...
        alpha_tensor = torch.ones((size[0], size[1], size[2], 1), device=t.device)
...
class MaskCombineOp:
    ...
    def combine(self, image1, image2, op, clamp_result, round_result):
        ...
        if clamp_result == "yes":
            result = torch.min(torch.max(result, torch.tensor(0., device=result.device)), torch.tensor(1., device=result.device))
        if round_result == "yes":
...
class UnaryMaskOp:
    ...
    def op_mask(self, image, op):
        ...
        if op == "invert":
            ...
            return (torch.min(torch.max(image, torch.tensor(0., device=image.device)), torch.tensor(1., device=image.device)),)
        elif op == "abs":
...
class MixColorByMask:
    ...
    def mix(self, image, r, g, b, mask):
        ...
        mask = tensor2batch(tensor2mask(mask), image.size()).to(image.device)
...
class CreateRectMask:
    ...
    def create_mask(self, mode, origin, x, y, width, height, image_width, image_height, copy_image_size = None):
        ...
        # Note: CreateRectMask has no image input to infer device; CPU is correct here
...
class MaskToRegion:
    ...
    def get_region(self, mask, padding, constraints, constraint_x, constraint_y, min_width, min_height, batch_behavior):
        ...
        # Account for padding
        min_x = torch.max(boxes[:,0] - padding, torch.tensor(0., device=mask.device))
        min_y = torch.max(boxes[:,1] - padding, torch.tensor(0., device=mask.device))
        max_x = torch.min(boxes[:,2] + padding, torch.tensor(mask_width, device=mask.device))
        max_y = torch.min(boxes[:,3] + padding, torch.tensor(mask_height, device=mask.device))

        width = max_x - min_x
        height = max_y - min_y

        # Make sure the width and height are big enough
        target_width = torch.max(width, torch.tensor(min_width, device=mask.device))
        target_height = torch.max(height, torch.tensor(min_height, device=mask.device))
        ...
        region = torch.zeros((mask_size[0], mask_height, mask_width), device=mask.device)
...
class CutByMask:
    ...
    def cut(self, image, mask, force_resize_width, force_resize_height, mask_mapping_optional = None):
        ...
        if mask_mapping_optional is not None:
            mask_mapping_optional = mask_mapping_optional.to(device)
        ...
        result = torch.zeros((B, use_height, use_width, 4), device=image.device)
        for i in range(0, B):
            if not is_empty[i]:
                ymin = int(min_y[i].item())
                ymax = int(max_y[i].item())
...
class SeparateMaskComponents:
    ...
    def separate(self, mask):
        ...
        result = torch.zeros([len(segments) - 1, H, W], device=mask.device)
        index = 0
...
class PasteByMask:
    ...
    def paste(self, image_base, image_to_paste, mask, resize_behavior, mask_mapping_optional = None):
        ...
        image_to_paste = image_to_paste.to(device)
        mask = mask.to(device)
        if mask_mapping_optional is not None:
            mask_mapping_optional = mask_mapping_optional.to(device)
        ...
        for i in range(0, MB):
            if is_empty[i]:
                ...
                pasting_alpha = torch.zeros([H, W], device=image_base.device)
...
class ConstantMask:
    ...
    def constant_mask(self, value, explicit_height, explicit_width, copy_image_size = None):
        ...
        dev = copy_image_size.device if copy_image_size is not None else 'cpu'
        result = torch.zeros([1, height, width], device=dev)
        result[:,:,:] = value
        return (result,)
...
class PruneByMask:
    ...
    def prune(self, image, mask):
        mask = tensor2mask(mask).to(image.device)
...
class MakeImageBatch:
    ...
    def append(self, image1, image2 = None, image3 = None, image4 = None, image5 = None, image6 = None):
        ...
        if image2 is not None:
            result = torch.cat((result, image2.to(result.device)), 0)
        if image3 is not None:
            result = torch.cat((result, image3.to(result.device)), 0)
        if image4 is not None:
            result = torch.cat((result, image4.to(result.device)), 0)
        if image5 is not None:
            result = torch.cat((result, image5.to(result.device)), 0)
        if image6 is not None:
            result = torch.cat((result, image6.to(result.device)), 0)
        return (result,)
...
NODE_CLASS_MAPPINGS = {
    ...
    "MasqueradeIncrementer": IncrementerNode,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "Mask By Text": "Mask By Text 311",
    "Mask Morphology": "Mask Morphology 311",
    "Combine Masks": "Combine Masks 311",
    "Unary Mask Op": "Unary Mask Op 311",
    "Unary Image Op": "Unary Image Op 311",
    "Blur": "Blur 311",
    "Image To Mask": "Image To Mask 311",
    "Mix Images By Mask": "Mix Images By Mask 311",
    "Mix Color By Mask": "Mix Color By Mask 311",
    "Mask To Region": "Mask To Region 311",
    "Cut By Mask": "Cut By Mask 311",
    "Paste By Mask": "Paste By Mask 311",
    "Get Image Size": "Get Image Size 311",
    "Change Channel Count": "Change Channel Count 311",
    "Constant Mask": "Constant Mask 311",
    "Prune By Mask": "Prune By Mask 311",
    "Separate Mask Components": "Separate Mask Components 311",
    "Create Rect Mask": "Create Rect Mask 311",
    "Make Image Batch": "Make Image Batch 311",
    "Create QR Code": "Create QR Code 311",
    "Convert Color Space": "Convert Color Space 311",
    "MasqueradeIncrementer": "Incrementer 311",
}
